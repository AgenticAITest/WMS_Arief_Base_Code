...
export const EditRelocationModal: React.FC<EditRelocationModalProps> = ({
  open,
  onOpenChange,
  relocationId,
  onSuccess,
    ...
    try {
      ...
      if (relocationResponse.data.success) {
        ...
        if (relocationData.status !== 'created') {
          toast.error('Only relocations with status "created" can be edited');
          onOpenChange(false);
          return;
        }

        setRelocation(relocationData);
        setNotes(relocationData.notes || '');
      }

      if (itemsResponse.data.success) {
        const items = itemsResponse.data.data || [];
        const mappedItems: RelocationItem[] = items.map((item: any) => ({
        ...
        setRelocationItems(mappedItems);
      }
    } catch (error) {
  ...
  const handleAddSelectedBins = () => {
    ...
    const newItems: RelocationItem[] = selectedBinData.map((bin: any) => ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `/api/modules/inventory-items/relocations/${relocationId}`,
        payload
      );
      ...
      toast.error(error.response?.data?.message || 'Failed to update relocation');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[90rem] sm:max-w-[90rem] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Relocation - {relocation?.relocationNumber}</DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleSubmit} disabled={submitting || relocationItems.length === 0}>
                {submitting ? 'Updating...' : 'Update Relocation'}
              </Button>
            </div>
          </div>
        ) : null}
