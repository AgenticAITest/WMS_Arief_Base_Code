...
- **GRN-Based Putaway Architecture**: Putaway system operates on GRN (receipt) level with 1:1 GRN-to-Putaway relationship. One PO can have multiple GRNs (partial receipts), thus multiple putaway operations. Putaway documents reference `receipt` type with receipt ID (not PO). The `purchase_orders_receipt` table includes `putawayStatus` field (`pending`, `completed`) to track individual GRN putaway state. Frontend displays GRN-centric accordions showing "GRN: XXX (PO: YYY)" for user clarity. Backend GET `/putaway` returns GRNs with joined PO details, and POST `/putaway/:id/confirm` accepts receipt ID to process putaway for that specific GRN.
- **Putaway Page Design**: Accordion-based layout with GRNs as collapsible sections (grouped by PO for user context). Item table includes cascading location dropdowns (Zone → Aisle → Shelf → Bin) that filter based on parent selections using `receiptItemId` for tracking. Backend includes `warehouseId` in API response to enable reliable cascading logic without fragile name-matching. Dropdowns reset dependent selections when parent changes to prevent stale data.
...
- **Confirm Putaway Flow**: GRN-based transaction workflow that validates all receipt items have bin assignments, creates `inventory_items` records with product and location data, generates PUTAWAY document referencing the GRN (stored as HTML in `storage/purchase-order/documents/tenants/{tenantId}/putaway/{year}/`), updates receipt `putawayStatus` to 'completed', and logs audit trail. Multiple putaways can occur per PO (one per GRN). Frontend modal displays GRN item summary with location paths and provides real-time feedback during confirmation.
...
- **Oct 29, 2025**:
  - **GRN-Based Putaway Architecture (MAJOR REFACTOR)**: Refactored entire Putaway system from PO-centric to GRN-centric architecture. Each GRN (receipt) now has its own 1:1 putaway document, allowing multiple putaways per PO when multiple partial receipts exist. Added `putawayStatus` enum field (`pending`, `completed`) to `purchase_orders_receipt` table. Updated GET `/putaway` endpoint to return GRNs with joined PO details instead of POs. Modified POST `/putaway/:id/confirm` to accept receipt ID, update receipt `putawayStatus`, and create putaway documents referencing receipts (not POs) via `referenceType: 'receipt'`. Updated `PutawayDocumentGenerator` to include GRN number and reference receipt ID. Completely refactored frontend `PurchaseOrderPutaway` to display GRN-based accordions (showing "GRN: XXX (PO: YYY)") with `receiptItemId` tracking throughout Smart Allocation and location dropdowns. Backend unique constraint `gen_docs_ref_idx` on `(tenant_id, reference_type, reference_id)` naturally supports multiple putaway documents per PO by referencing different receipt IDs.
